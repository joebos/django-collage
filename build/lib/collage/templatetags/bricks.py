...
class Collage(object):
        ...
        for idx, image in enumerate(self.images):
            try:
                if image.find("http") >= 0:
                    image_local = self.download_image(image)
                    images[idx] = image_local
                    image = images[idx]
    ...
    def download_image(self, src_url):
        import uuid
        local_image_file_name = '%s%s' % (settings.MEDIA_URL, str(uuid.uuid1()) + ".jpg")
        import urllib
        urllib.urlretrieve(src_url, local_image_file_name)
        return local_image_file_name
